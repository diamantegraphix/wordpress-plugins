...
/**
 * @file activation_deactivation_hook.php
 *
 * Helper file for handling action and deaction of plugins.
 * On activation we have to create database tables and on
 * deactivation we to drops the created tables.
 */
...
function create_estatik_tables(){
    ...
    create_or_update('estatik_settings', array(
        ...
        'property_slug' => "varchar(255) NOT NULL DEFAULT '0'",
    ...
    $date_format = $wpdb->get_var("SELECT date_format FROM {$wpdb->prefix}estatik_settings WHERE `setting_id`='1'");

    if ( empty($date_format) ) {
        $wpdb->query("UPDATE {$wpdb->prefix}estatik_settings SET `date_format`='d/m/y' WHERE `setting_id`='1'");
    }

    $property_slug = $wpdb->get_var("SELECT property_slug FROM {$wpdb->prefix}estatik_settings WHERE `setting_id`='1'");

    if ( empty($property_slug) ) {
        $wpdb->query("UPDATE {$wpdb->prefix}estatik_settings SET `property_slug`='properties' WHERE `setting_id`='1'");
    }
