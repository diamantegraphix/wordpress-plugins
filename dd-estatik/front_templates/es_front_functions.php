...
function estatik_front_scripts() {
    ...
    if (!empty($es_settings->theme_style)) {
    ...
    if ( !wp_script_is('es-google-maps', 'enqueued') ) {
        wp_enqueue_script('es-google-maps', 'http://maps.googleapis.com/maps/api/js?sensor=false' , array('jquery'));
    }

    if ( !wp_script_is('es-google-maps-key', 'enqueued') ) {
        wp_enqueue_script('es-google-maps-key', 'https://maps.googleapis.com/maps/api/js?key=AIzaSyAN_y1mOXhfEIR23AbtbWwgP89EiClvJ74');
    }
...
function es_print_scripts() {
    ...
    if (!empty($es_settings->prop_singleview_photo_thumb_width) && !empty($es_settings->prop_singleview_photo_thumb_width)) {
...
function es_google_map() {
    ...
    if(is_singular('properties')){
        ...
        $es_prop_single = !empty($es_prop_single_result[0]) ? $es_prop_single_result[0] : null;


        if (!empty($es_prop_single)) {
...
function es_search_input_select($title, $name, $table) {
